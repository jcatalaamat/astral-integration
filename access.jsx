...
const certColor = id => CERTS.find(c => c.id === id)?.color || "#666";
const certName = id => CERTS.find(c => c.id === id)?.name || id;
const certAbbr = id => CERTS.find(c => c.id === id)?.abbr || id;
const haversine = (a, b, c, d) => { const R=6371, p=Math.PI/180, dl=(c-a)*p, dn=(d-b)*p; const x=Math.sin(dl/2)**2+Math.cos(a*p)*Math.cos(c*p)*Math.sin(dn/2)**2; return R*2*Math.atan2(Math.sqrt(x),Math.sqrt(1-x)); };
const fmtDist = d => d<100?`${Math.round(d)} km`:d<1000?`${Math.round(d)} km`:`${(d/1000).toFixed(1)}k km`;

const Card = ({ f, loc, onProfile, onBook }) => {
      ...
      <div style={{ display:"flex", gap:18, alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:"flex", gap:5, marginTop:10, flexWrap:"wrap" }}>
            {f.certs.slice(0, open ? undefined : 4).map(c => (
      ...
      {open && (
        <div style={{ marginTop:20, paddingTop:16, borderTop:"1px solid #f0f2f1" }}>
          ...
          <div style={{ display:"flex", gap:24, flexWrap:"wrap", marginBottom:14 }}>
            ...
            <div><div style={{ fontSize:10, color:"#8a9a9a", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:4, fontFamily:"'DM Mono',monospace" }}>Certifications</div><div style={{ fontSize:13, color:"#3a4a4a" }}>{f.certs.map(c => certName(c)).join(", ")}</div></div>
          </div>
